$( document ).ready(function() {
    ...
    var employeeName = sessionStorage.getItem("name")
    if (employeeName == null) {
      alert("Invalid Page Access, Nothing is Going to Work")
    }
    $('#radio').text("Welcome " + employeeName)
...
// http://127.0.0.1:5000/
    ...
    for (i = 0; i < arr.revisions.length; i++) {
        ...
        specDict['content'] = newFilteredString
        specDict['timestamp'] = timestamp
        specDict['selected'] = 0
        revisionArray.push(specDict)
...
var checkBoxThingy = newCommitThingy[0].querySelector('td input');
checkBoxThingy.onclick = function() {
  var someNumber = specDict['selected']
  specDict['selected'] = 1 - someNumber
}
...
$('#id01').append(newCommitThingy)
    ...
    sessionStorage.setItem("revisionArray", revisionArray)
